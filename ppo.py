...
def createTeam(firstIndex, secondIndex, isRed,
  ...
  m1,a1 = mlp(13,[10],5), mlp(13,[10],1)
  m2,a2 = mlp(13,[10],5), mlp(13,[10],1)
...
class PPOAgent(Agent1):
    ...
    def chooseAction(self, gameState: capture.GameState, policy=None):
        actions = gameState.getLegalActions(self.index)
        # print(f"AGENT LEGAL ACTIONS {actions}")
        rng = np.random.default_rng()
        return rng.choice(actions), 0
    ...
    def rollout(self):
        return torch.tensor(np.array(self.batch_obs),dtype=float), \
                torch.tensor(np.array(self.batch_acts),dtype=float), \
                torch.tensor(np.array(self.batch_log_probs),dtype=float), \
                torch.tensor(np.array(self.batch_rtgs),dtype=float), \
                self.batch_lens 
